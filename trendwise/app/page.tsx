        ...
        {loading ? (
          ...
          <div className="grid gap-8 md:gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
            {filteredArticles.map((article) => (
              <ArticleCard
                key={article._id}
                id={article.slug ? article.slug : article._id}
                title={article.title}
                excerpt={article.excerpt}
                coverImage={article.coverImage ? article.coverImage : '/default-cover.jpg'}
                category={article.category ? article.category : 'General'}
                readTime={article.readTime ? article.readTime : 'N/A'}
                publishedAt={formatDate(article.createdAt)}
                author={article.author ? article.author : { name: 'Unknown', avatar: '/default-avatar.svg' }}
              />
            ))}
          </div>
        )}
